<script lang="ts">
  ...
  const revenueData = [
    {
      category: "Fantasy League Fees",
      values: [1500000, 2250000, 3000000]
    }
  ];
  ...
  onMount(() => {
    const svg = d3.select('#fantasy-revenue-chart')
      .attr('width', width)
      .attr('height', height);
...
// Modify the bars section to include tooltip
revenueData.forEach((category, i) => {
  svg.selectAll(`.bar-${i}`)
    ...
    svg.append("text")
      ...
      .style("fill", "currentColor")
      .text("Fantasy League Revenue Projections (2026-2028)");
  });
</script>

<div class="chart-container">
  <svg id="fantasy-revenue-chart"></svg>
</div>
