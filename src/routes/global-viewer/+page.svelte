<script lang="ts">
  ...
  import { Home, Globe, TrendingUp, MapPin, Users, Target, BarChart3, LineChart, Map, Flag, Tv, Eye } from "lucide-svelte";
  import Breadcrumb from "$lib/components/ui/breadcrumb/breadcrumb.svelte";
  import { pb } from '$lib/pocketbase';

  function getIconForSection(name: string) {
    const nameLower = name.toLowerCase();
    if (nameLower.includes('global') || nameLower.includes('overview')) return Globe;
    if (nameLower.includes('participation') || nameLower.includes('course') || nameLower.includes('footprint')) return MapPin;
    if (nameLower.includes('engagement') || nameLower.includes('numbers')) return BarChart3;
    if (nameLower.includes('viewer') || nameLower.includes('opportunity') || nameLower.includes('viewership')) return Eye;
    if (nameLower.includes('strategic') || nameLower.includes('distribution') || nameLower.includes('implications')) return Target;
    if (nameLower.includes('takeaway') || nameLower.includes('key')) return LineChart;
    if (nameLower.includes('projected') || nameLower.includes('market')) return Map;
    if (nameLower.includes('credible') || nameLower.includes('estimates')) return Tv;
    if (nameLower.includes('country') || nameLower.includes('united states') || nameLower.includes('finland')) return Flag;
    return Users;
  }
  ...
  async function fetchGlobalRecords() {
    try {
      const response = await fetch('/api/global-viewer', {
        headers: {
          'Authorization': `Bearer ${pb.authStore.token}`
        }
      });
      const data = await response.json();
      globalRecords = data.items;
      console.log("✅ Global Viewer Records Updated!");
    } catch (error) {
      console.error("❌ ERROR: Failed to fetch global viewer records", error);
    }
  }
  ...
  <aside class="bg-muted/40 border-r hidden md:flex flex-col">
    <div class="h-14 border-b px-4 flex items-center lg:px-6">
      <div class="flex items-center gap-2">
        <Globe class="h-5 w-5" />
        <span class="font-semibold text-xl">Global Viewer</span>
      </div>
    </div>
      ...
      <div class="w-full flex-1 flex items-center gap-4">
        <form class="flex-1">
          <div class="relative">
            ...
            <Input
              type="search"
              placeholder="Search global viewer data..."
    ...
    <main class="flex flex-1 flex-col gap-4 p-4 lg:gap-6 lg:p-6">
      <Breadcrumb
        items={[
          ...
          { label: 'Global Viewer', href: '/global-viewer' }
        ]}
      />

      <div class="text-center mb-8">
        <div class="inline-flex items-center justify-center w-20 h-20 rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 shadow-xl mb-4">
          <Globe class="h-10 w-10 text-white" />
        </div>
        <h1 class="text-4xl font-bold mb-2">
          Global Viewer & Participation Report
        </h1>
        <div class="h-1 w-32 bg-gradient-to-r from-emerald-500 to-teal-500 rounded-full mx-auto"></div>
      </div>
        ...
        <div class="grid gap-6 md:grid-cols-1 lg:grid-cols-1">
          {#each filteredRecords as record}
            <div id={`section-${record.id}`} class="border-2 rounded-2xl overflow-hidden hover:shadow-2xl hover:scale-[1.02] transition-all bg-card">
              <div class="bg-gradient-to-r from-emerald-600 via-emerald-300 to-teal-100 dark:from-emerald-950/30 dark:to-teal-950/30 p-6 border-b-2">
                <div class="flex items-center gap-4">
                  <div class="shrink-0">
                    <div class="h-16 w-16 rounded-2xl bg-gradient-to-br from-emerald-600 to-teal-500 flex items-center justify-center shadow-xl">
                  ...
                  <div>
                    ...
                    <div class="h-1 w-20 bg-gradient-to-r from-white to-teal-200 rounded-full mt-2"></div>
                  </div>
                </div>
              </div>
      ...
      {:else}
        <div class="text-center py-12">
          ...
          <p class="text-muted-foreground text-lg">No global viewer data found</p>
        </div>
      {/if}
    </main>
  </div>
