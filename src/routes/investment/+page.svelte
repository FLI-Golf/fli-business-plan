<script lang="ts">
  ...
  async function fetchInvestmentRecords() {
    try {
      ...
      investmentRecords = data.items;
      console.log("✅ Investment Records Updated!");
    } catch (error) {
      console.error("❌ ERROR: Failed to fetch investment records", error);
    }
  }
  ...
  $: filteredRecords = investmentRecords.filter(record =>
  ...
  <aside class="bg-muted/40 border-r hidden md:flex flex-col">
    <div class="h-14 border-b px-4 flex items-center lg:px-6">
      <div class="flex items-center gap-2">
        ...
        <span class="font-semibold text-xl">Investment Overview</span>
      </div>
    </div>

    <nav class="flex-1 overflow-y-auto p-4">
      {#if investmentRecords.length}
        <div class="space-y-2">
          {#each investmentRecords as record}
            <div class="space-y-1">
              <a
      ...
      <div class="w-full flex-1 flex items-center gap-4">
        <form class="flex-1">
          <div class="relative">
            ...
            <Input
              type="search"
              placeholder="Search investment content..."
