<script lang="ts">
  ...
  async function fetchTechRecords() {
    try {
      const records = await pb.collection('tech').getList(1, 50, {
        sort: 'order'
      });
      techRecords = records.items;
      console.log("✅ Technology Records Updated!");
    } catch (error) {
      console.error("❌ ERROR: Failed to fetch technology records", error);
    }
  }
  ...
  $: filteredRecords = techRecords.filter(record =>
  ...
  <aside class="bg-muted/40 border-r hidden md:flex flex-col">
    <div class="h-14 border-b px-4 flex items-center lg:px-6">
      <div class="flex items-center gap-2">
        <Cpu class="h-5 w-5" />
        <span class="font-semibold text-xl">Technology & Innovation</span>
      </div>
    </div>
    <nav class="flex-1 overflow-y-auto p-4">
      {#if techRecords.length}
        <div class="space-y-2">
          {#each techRecords as record}
            <div class="space-y-1">
              <a
      ...
      {:else}
        <p class="text-sm text-muted-foreground">No technology records available</p>
      {/if}
    </nav>
  </aside>
